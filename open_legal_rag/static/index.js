...
chatInput.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    for (const context of data.context) {
      ...
      <h3>${sanitizeString(context.case_name)}</h3>

      <table>
        <tr>
          <td>Case name: </td>
          <td><span>${sanitizeString(context.case_name, false)}</span>
          </td>
        </tr>

        <tr>
          <td>Date filed: </td>
          <td><span>${sanitizeString(context.case_date_filed, false)}</span></td>
        </tr>

        <tr>
          <td>Court: </td>
          <td>
            <span>${sanitizeString(context.court_name, false)}</span>
          </td>
        </tr>
      </table>
      
      <textarea disabled>${sanitizeString(context.opinion_text, false)}</textarea>
      `;
    }
