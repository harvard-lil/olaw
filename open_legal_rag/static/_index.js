...
const renderChatBubble = (type, message, metadata) => {
  switch (type) {
    ...
    case "confirm-search":
      const searchTargetName =
        searchTarget === "courtlistener" ? "the Court Listener API" : "";
...
const streamCompletion = async () => {
  ...
  const maxTokens =
    maxTokensInput.value === "" ? null : parseInt(maxTokensInput.value);
  ...
  if (response.status != 200) {
    renderChatBubble(
      "error",
      `Could not communicate with ${sanitizeString(model)}.`
    );
    unlock();
    return;
  }
  ...
  // Append "ai" bubble
  renderChatBubble("ai", "");
  const aiBubbleText = chatConversation.querySelector(
...
chatInput.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const searchStatementResponse = await fetch(
...
chatConversation.addEventListener("click", async (e) => {
  ...
  if (
    !e.target.classList.contains("confirm-search-yes") &&
...
setInterval(() => {
  ...
  const maxTokensOk =
    maxTokensInput.value === "" || maxTokensInput.value.match(/[0-9]+/) != null;
...
document.querySelector("body").addEventListener("click", (e) => {
  ...
  if (
    ...
    e.target.parentNode instanceof HTMLDialogElement
  ) {
